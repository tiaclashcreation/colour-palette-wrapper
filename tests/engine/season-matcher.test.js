...
test("scoreItemForSeason returns match for aligned palette", () => {
  const item = {
...
test("scoreItemForSeason returns match for single palette hit", () => {
  const item = {
...
test("summarizeResults aggregates buckets", () => {
  ...
  assert.deepEqual(summary, { scanned: 3, match: 2, noMatch: 1 });
});
